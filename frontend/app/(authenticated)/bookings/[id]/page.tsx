...
export default function OrderDetailsPage() {
  ...
  const auth = useAuth();
  // (authenticated)/layout.tsx guarantees auth.user is set before rendering.
  // The guard below satisfies TypeScript's type narrowing — no ! or ?? needed.
    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      <div className="grid grid-cols-3 gap-8">
        ...
        <div className="col-span-2 space-y-4">
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {/* Hide edit button if product refund is completed or security has been paid */}
                    {!hasProductRefund(product.id) && !(securityByProduct[product.id] > 0) && (
                      <button
          ...
          {/* Inactive (cancelled / exchanged) products — admin only, greyed out, no actions */}
          {inactiveProducts.length > 0 && (
            <>
              ...
              {inactiveProducts.map((product: any, index: number) => (
                <div
                  key={`inactive-${index}`}
                  className="bg-gray-50 border border-gray-200 rounded-lg p-6 flex gap-6 opacity-50 pointer-events-none select-none"
                >
                  <div className="w-32 h-40 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                    {(() => {
                      const imageData = product.image || product.imageUrl || product.rawImage;
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className="text-xl font-semibold text-gray-500">{product.name}</h3>
                      <ProductStatusBadge status={product.status} size="md" />
                    </div>
                    {product.code && (
        ...
        <div className="space-y-6">
          {/* Document Generation Buttons — admin only */}
          ...
          {(currentUser.role === 'admin' || salesmanPermissions.cancellation_allowed) && booking && (
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              ...
              <BookingCancellation
                ...
                userRole={currentUser.role}
                userName={userName}
                canCancel={currentUser.role === 'admin' || salesmanPermissions.cancellation_allowed}
              />
            </div>
          )}
