...
// POST /api/setup/credit-notes - Create credit notes table
router.post('/credit-notes', async (req, res) => {
  try {
    ...
    res.json({ 
      success: true, 
      message: 'Credit notes table created successfully' 
    });
  } catch (error) {
    console.error('Error creating credit notes table:', error);
    res.status(500).json({ 
      success: false, 
      error: 'Failed to create credit notes table',
